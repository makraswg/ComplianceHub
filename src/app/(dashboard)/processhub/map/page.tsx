...
/**
 * Erzeugt das XML für die Prozesslandkarte.
 * Stellt Prozesse als Knoten und ihre Verknüpfungen als Kanten dar.
 */
function generateMapXml(processes: Process[], relations: { fromId: string; toId: string; label: string }[]) {
  ...
  relations.forEach((rel, idx) => {
    // Nur Kanten zeichnen, wenn beide Endpunkte in der aktuellen Auswahl sind
...
export default function ProcessMapPage() {
  ...
  // Selection State für selektive Karte
  ...
  // Alle Relationen (Handover) extrahieren
    ...
    <div className="h-[calc(100vh-120px)] flex flex-col -m-8 overflow-hidden bg-slate-50 font-body">
      <header className="h-16 border-b bg-white flex items-center justify-between px-8 shrink-0 z-20 shadow-sm">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => router.push('/processhub')} className="h-10 w-10 text-slate-400 hover:bg-slate-100 rounded-none">
          ...
          <div>
            <h1 className="text-lg font-black uppercase tracking-tight text-slate-900">Prozesslandkarte</h1>
            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-0.5">Unternehmensweite Gesamtsicht & Vernetzung</p>
          </div>
        </div>

        <Tabs value={activeTab} onValueChange={setActiveTab} className="bg-slate-100 p-1 rounded-none border">
          <TabsList className="bg-transparent h-9 gap-1">
            <TabsTrigger value="cards" className="rounded-none text-[10px] font-bold uppercase data-[state=active]:bg-white data-[state=active]:shadow-sm">
              <Layers className="w-3.5 h-3.5 mr-2" /> Kachel-Ansicht
            </TabsTrigger>
            <TabsTrigger value="diagram" className="rounded-none text-[10px] font-bold uppercase data-[state=active]:bg-white data-[state=active]:shadow-sm">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-80 border-r bg-white flex flex-col shrink-0">
          <div className="p-6 space-y-6">
            <div className="space-y-4">
              ...
              <div className="relative">
                ...
                <Input 
                  ...
                  className="pl-9 h-10 text-xs rounded-none border-2 bg-slate-50/50"
                  value={search}
            ...
            <div className="space-y-4 pt-4 border-t">
              <div className="flex items-center justify-between">
                <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Selektive Auswahl</Label>
                ...
                <div className="space-y-1">
                  {filteredProcesses.map(proc => (
                    <div 
                      key={proc.id} 
                      className={cn(
                        "flex items-center gap-3 p-2.5 border transition-all cursor-pointer bg-white group",
                    ...
                    >
                      <Checkbox checked={selectedProcessIds.includes(proc.id)} className="rounded-none shrink-0" />
                      <div className="min-w-0 flex-1">
                        <p className="text-[11px] font-bold text-slate-800 truncate leading-tight">{proc.title}</p>
                        <p className="text-[8px] font-black uppercase text-slate-400 mt-0.5">V{proc.currentVersion} • {proc.status}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="mt-auto p-6 bg-slate-50 border-t">
            <div className="p-4 bg-white border-2 border-dashed border-slate-200 text-center space-y-2">
              <p className="text-[10px] font-black uppercase text-slate-400">Statistik</p>
              <div className="grid grid-cols-2 gap-4">
                <div><p className="text-xl font-black text-slate-900">{filteredProcesses.length}</p><p className="text-[8px] font-bold uppercase text-slate-400">Gesamt</p></div>
                <div><p className="text-xl font-black text-blue-600">{allRelations.length}</p><p className="text-[8px] font-bold uppercase text-slate-400">Links</p></div>
              </div>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 bg-slate-100 relative overflow-hidden">
          <TabsContent value="cards" className="h-full m-0 p-8 overflow-y-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {mapProcesses.map(proc => {
                ...
                return (
                  <Card key={proc.id} className="rounded-none border-2 shadow-none hover:border-primary/50 transition-all group flex flex-col bg-white">
                    ...
                    <CardContent className="p-6 flex-1 flex flex-col space-y-6">
                      ...
                      <div className="pt-4 border-t">
                        <Button 
                          className="w-full h-10 rounded-none bg-slate-900 hover:bg-black text-white text-[10px] font-black uppercase tracking-widest gap-2 shadow-lg"
          ...
          <TabsContent value="diagram" className="h-full m-0 p-6 flex flex-col">
            <div className="absolute top-10 right-10 z-10 bg-white/95 backdrop-blur shadow-2xl border p-1.5 flex flex-col gap-1.5">
              <TooltipProvider>
                <Tooltip><TooltipTrigger asChild><Button variant="ghost" size="icon" onClick={syncDiagram} className="h-9 w-9"><RefreshCw className="w-4 h-4 text-slate-600" /></Button></TooltipTrigger><TooltipContent side="left" className="text-[10px] font-bold uppercase">Sync Diagramm</TooltipContent></Tooltip>
            ...
            <div className="p-4 bg-slate-900 text-white flex items-center justify-between shrink-0">
              <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest">
                <div className="flex items-center gap-2"><div className="w-3 h-3 bg-emerald-500 rounded-none" /> Veröffentlicht</div>
                <div className="flex items-center gap-2"><div className="w-3 h-3 bg-white border border-slate-400 rounded-none" /> Entwurf</div>
                <div className="flex items-center gap-2 text-blue-400"><ArrowRight className="w-3 h-3" /> Handover / Relation</div>
              </div>
              <p className="text-[9px] font-bold text-slate-400 italic">Visualisierung der Prozesslandschaft basierend auf Ihrer aktuellen Auswahl.</p>
            </div>
          </TabsContent>
        </main>
      </div>
      ...
      {isProcLoading && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white animate-pulse">Lade Prozesslandschaft...</p>
        </div>
      )}
    </div>
  );
