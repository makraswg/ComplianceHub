...
export default function UsersPage() {
  ...
  const openAdvisor = async (userDoc: any) => {
    ...
    try {
      const userAssignments = assignments?.filter(a => a.userId === userDoc.id) || [];
          ...
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow className="hover:bg-transparent">
                ...
                <TableHead className="font-bold uppercase tracking-widest text-[10px]">Risk Profile</TableHead>
                <TableHead className="text-right font-bold uppercase tracking-widest text-[10px]">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredUsers?.map((user) => {
                const userEnts = assignments?.filter(a => a.userId === user.id && a.status !== 'removed').length || 0;
                return (
                  <TableRow key={user.id} className="group hover:bg-muted/5 border-b">
                    ...
                    <TableCell>
                      <div className="flex items-center gap-1.5 font-bold text-[10px] uppercase text-slate-600">
                        <ShieldCheck className="w-3 h-3" /> {userEnts} Entitlements
                      </div>
                    </TableCell>
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-56 rounded-none p-1 shadow-xl">
                          <DropdownMenuItem onSelect={() => { setSelectedUser(user); setTimeout(() => setIsProfileOpen(true), 150); }}>
                            <UserCircle className="w-3.5 h-3.5 mr-2" /> Identity Profile
                          </DropdownMenuItem>
                          <DropdownMenuItem onSelect={() => router.push(`/assignments?search=${user.displayName}`)}>
                            <ShieldCheck className="w-3.5 h-3.5 mr-2" /> View Access
                          </DropdownMenuItem>
                          <DropdownMenuItem className="text-blue-600 font-bold" onSelect={() => openAdvisor(user)}>
                            <BrainCircuit className="w-3.5 h-3.5 mr-2" /> AI Access Advisor
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
        ...
        <DialogContent className="max-w-xl rounded-none border shadow-2xl">
          <DialogHeader className="border-b pb-4">
            <div className="flex items-center gap-2 text-blue-600">
              ...
              <DialogTitle>AI Access Assessment</DialogTitle>
            </div>
          </DialogHeader>
                    ...
                    <ul className="space-y-2">
                      {aiAdvice.recommendations.map((rec: string, i: number) => (
                        <li key={i} className="text-xs bg-blue-50/50 p-2 border-l-2 border-blue-400 flex items-start gap-2">
                          <CheckCircle className="w-3 h-3 mt-0.5 text-blue-600 shrink-0" /> {rec}
                        </li>
                      ))}
                    </ul>
                  </div>
