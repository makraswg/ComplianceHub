...
export default function PoliciesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.success && importFile) {
        // Handle Quick Start Import
        const reader = new FileReader();
        reader.onload = async (e) => {
          const base64 = e.target?.result as string;
          // Create a version from the imported file (simulated text extraction for MVP)
  ...
  const getTypeName = (type: Policy['type']) => {
    const map = { DA: 'Dienstanweisung', BV: 'Betriebsvereinbarung', ISK: 'Sicherheitskonzept', DS: 'Datenschutz' };
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b pb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="text-2xl font-headline font-bold text-slate-900 dark:text-white uppercase tracking-tight">Richtlinien-Register</h1>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Zentrale Verwaltung und Versionierung von Vorgabedokumenten.</p>
          </div>
        </div>
        <Button size="sm" className="h-9 rounded-xl font-bold text-xs px-6 bg-emerald-600 hover:bg-emerald-700 text-white shadow-lg active:scale-95 transition-all" onClick={() => { resetForm(); setIsDialogOpen(true); }}>
          <Plus className="w-3.5 h-3.5 mr-2" /> Neue Richtlinie
        </Button>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border shadow-sm overflow-hidden">
        {isLoading ? (
          ...
          <div className="py-20 text-center space-y-4 opacity-30">
            ...
            <p className="text-xs font-black uppercase">Keine Richtlinien gefunden</p>
          </div>
        ) : (
          <Table>
                ...
                <TableRow key={p.id} className="group hover:bg-slate-50 transition-colors border-b last:border-0 cursor-pointer" onClick={() => router.push(`/policies/${p.id}`)}>
                  <TableCell className="py-4 px-6">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-100 shadow-inner">
...
function getTenantSlug(id?: string | null) {
  return id || 'global';
}
