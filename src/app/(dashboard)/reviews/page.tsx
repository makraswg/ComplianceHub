...
export default function AccessReviewsPage() {
  ...
  const handleReview = async (assignmentId: string, action: 'certify' | 'revoke') => {
    ...
    try {
      ...
      const userDoc = users?.find((u: any) => u.id === existing.userId);
      const ent = entitlements?.find((e: any) => e.id === existing.entitlementId);
      const res = resources?.find((r: any) => r.id === ent?.resourceId);

      await logAuditEventAction(dataSource, {
        tenantId: existing.tenantId || 'global',
        actorUid: user?.email || 'system',
        action: `Review abgeschlossen (${action === 'certify' ? 'Zertifiziert' : 'Widerrufen'}): ${userDoc?.displayName} -> ${res?.name}/${ent?.name}`,
                  ...
                  return (
                    <TableRow key={a.id} className="group hover:bg-slate-50 transition-colors border-b last:border-0">
                      ...
                      <TableCell className="text-center">
                        <button 
                          className="h-8 rounded-md text-[10px] font-bold text-indigo-600 hover:bg-indigo-50 gap-2 border border-transparent hover:border-indigo-100 transition-all flex items-center px-3 mx-auto"
